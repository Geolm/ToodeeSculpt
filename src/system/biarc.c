...
//----------------------------------------------------------------------------------------------------------------------------------------------------------
void biarc_recursive(vec2 c0, vec2 c1, vec2 c2, uint32_t max_subdivision, float error_max, struct arc* arcs, uint32_t *num_arcs, uint32_t current_subdivision)
{
    vec2 incenter = triangle_incenter(c0, c1, c2);
    vec2 split = vec2_add(vec2_add(vec2_scale(c0, .25f), vec2_scale(c1, .5f)), vec2_scale(c2, .25f));
    vec2 split_tangent = vec2_sub(c2, c0);

    if (vec2_colinear(c0, c1, c2, 0.0001f))
    {
    ...
    if ((vec2_sq_distance(split, incenter) > float_square(error_max)) && (current_subdivision < max_subdivision))
    {
        ...
        {
            biarc_recursive(c0, mid_left, split, max_subdivision, error_max, arcs, num_arcs, current_subdivision+1);
            biarc_recursive(split, mid_right, c2, max_subdivision, error_max, arcs, num_arcs, current_subdivision+1);
        }
    }
    else
    {
...
void biarc_tessellate(vec2 p0, vec2 p1, vec2 p2, uint32_t max_subdivision, float error_max, struct arc* arcs, uint32_t *num_arcs)
{
    ...
    *num_arcs = 0;
    biarc_recursive(c[0], c[1], c[2], max_subdivision, error_max, arcs, num_arcs, 1);
}